...
import com.bupt.wangfu.info.entry.Queue;
...
import com.bupt.wangfu.mgr.admin.SdnConfig;
...
public class PSManagerUI {
	...
	private ArrayList<WsnPolicyMsg> policyList;
	private ArrayList<MsgException> msgExceptions;
	private Data data = new Data();// 数据类
	private AdminMgr adminMgr;// 系统管理员
	private GlobalUtil globalUtil; // 全局拓扑
	...
	private List<TargetGroup> currentTargetGroups;// 受限集群框内
	// 被选中的集群名称和
	...
	public PSManagerUI(AdminMgr adminMgr, GlobalUtil globalUtil) {
	...
	public void open() {
		...
		checkButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (searchInput.equals("")) {
					...
					ArrayList<String> sercherSubs = groupSubs.get(searchInput);
		...
		if (AdminMgr.groups != null) {
			for (final String groupName : AdminMgr.groups.keySet()) {
					...
					public void actionPerformed(ActionEvent e) {
						if (groups.isSelected()) {
							...
							for (TargetGroup temTG : currentTargetGroups) {
								if (temTG.getName().equals(groupName)) {
		...
		confirmFbdnGroups.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				List<TargetGroup> currentChosenTargetGroups = new ArrayList<>();
				...
				for (int i = 0; i < fbdnGroupsPanel.getComponentCount(); i++) {
					...
					if (topicsChoosed.isSelected()) {
						TargetGroup currentChosenTargetGroup = new TargetGroup(topicsChoosed.getText());
				...
				for (int i = 0; i < topicsPanel.getComponentCount(); i++) {
					...
					if (topicsChoosed.getSelectedIndex() != 0 && topicsChoosed.getSelectedItem() != null) {
						...
						currentChosenTopicEntry.setTopicName(topicsChoosed.getSelectedItem().toString());
						chosenTopicPath = "ou="+ topicsChoosed.getSelectedItem().toString() + "," + chosenTopicPath;
						try {
							currentChosenTopicEntry.setTopicCode(ShorenUtils.ldap.getByDN(chosenTopicPath).getTopicCode());
				...
				} else if (currentChosenTopicEntry.getTopicPath() != null) {
	...
	public void reflashFbdnGroups() {
		if (comboBox.getSelectedIndex() != 0 && comboBox.getSelectedItem() != null && !comboBox.getSelectedItem().toString().equals("null")) {
			...
			if (currentChosenTopicEntry.getWsnpolicymsg() != null) {
				WsnPolicyMsg selectedPolicy = ShorenUtils.decodePolicyMsg(currentChosenTopicEntry);

				List<TargetGroup> targetGroups = selectedPolicy.getTargetGroups();
	...
	public void reflashCurrentPolicy() {
		...
		for (WsnPolicyMsg aPolicyList : policyList) {
	...
	public void reloadAllGroup() {
		...
		if (AdminMgr.groups != null && AdminMgr.groups.size() > 0) {

			for (final String groupName : AdminMgr.groups.keySet()) { // 遍历当前所有集群
				...
				groupButton.addMouseListener(new MouseAdapter() {
					public void mouseClicked(MouseEvent e) {
						if (e.getButton() == MouseEvent.BUTTON1) {
							...
							String groupControllerAddr = AdminMgr.groups.get(currentGroup).controllerAddr;
							...
							if (groupController == null) {
								...
								while (n == 0) {
									...
									groupController = globalUtil.controllers.get(groupControllerAddr);
									if (groupController != null) {
										break;
									}
								}
								if (n == 2) {
									AdminMgr.groups.remove(currentGroup);
									reloadAllGroup();
								}
								...
								ctrlInfo.setText("集群" + currentGroup + "当前控制器URL: " + groupControllerAddr);
								...
								//获取集群所有交换机和群内拓扑
								final Map<String, Switch> groupSwitchMap = globalUtil.getAllSwitch(groupController);
								...
								for (final String switchName : groupSwitchMap.keySet()) {
									...
									final Controller finalGroupController = groupController;
									...
									groupSwitchButton.addMouseListener(new MouseAdapter() {
										...
										public void mouseClicked(MouseEvent e) {
											if (e.getButton() == MouseEvent.BUTTON1) {
												...
												Map<String, WSNHost> wsnHostMap = globalUtil.getHostsOfSwitch(groupSwitchMap.get(switchName));
												for (final WSNHost host : wsnHostMap.values()) {
													final String memAddr = host.getIpAddr();
											...
											if (e.getButton() == MouseEvent.BUTTON3) {
												...
												portInfoMenu.addActionListener(new ActionListener() {
													...
													public void actionPerformed(ActionEvent e) {
														...
														List<Port> pList = globalUtil.getPortsOfSwitch(finalGroupController, groupSwitchMap.get(switchName));
														...
														portTable.addMouseListener(new MouseAdapter() {
															public void mouseClicked(MouseEvent e) {
																...
																List<Queue> qList = globalUtil.getQueuesOfPort(finalGroupController, groupSwitchMap.get(switchName), pArray[row]);
							...
							if (groupSubs != null) {
								int count = 0;
								for (String temp : groupSubs.keySet()) {
									subers.add(temp);
									count += groupSubs.get(temp).size();
								}
								String[][] groupSubData = new String[count][2];
								ArrayList<String> groupAllTopics = new ArrayList<>();
								for (String temp : subers) {
									groupAllTopics.addAll(groupSubs.get(temp));
								}
								for (int j = 0; j < subers.size(); j ++) {
									groupSubData[j][0] = subers.get(j);
								}
								for (int j = 0; j < groupAllTopics.size(); j++) {
									groupSubData[j][1] = groupAllTopics.get(j);
								}

								String[] columnNames = {"集群" + currentGroup + "的所有订阅用户","集群" + currentGroup + "的所有订阅"};
				...
				groups.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (groups.isSelected()) {
							TargetGroup tem = new TargetGroup(currentGroup);
							currentTargetGroups.add(tem);
						} else {
							for (TargetGroup temTG : currentTargetGroups) {
								if (Objects.equals(temTG.getName(), currentGroup)) {
									currentTargetGroups.remove(temTG);
									break;
								}
							}
